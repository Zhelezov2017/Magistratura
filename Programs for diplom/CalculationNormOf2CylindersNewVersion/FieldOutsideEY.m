function FieldOutsideEY = FieldOutsideEY(N_max, Const,k_0, p, x0, y0, x, y)
m  = [-N_max:N_max];
if(N_max == 0) m = 0; end
q = sqrt(1 - p.^2);
q = q.* (2*(imag(q) <= 0)-1);

FieldOutsideEPHI = zeros(1001,1001);
FieldOutsideERHO = zeros(1001,1001);
for ii = 1:1001
    for ij = 1:1001
        dx = x(ii)-x0;
        dy = y(ij)-y0;
        ...
        FieldOutsideEY(ii,ij) = FieldOutsideERHO(ii,ij)*sin(phi)+ FieldOutsideEPHI(ii,ij)*cos(phi);
    end
end
