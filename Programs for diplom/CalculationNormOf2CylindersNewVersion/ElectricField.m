function ElectricField = ElectricField(N_max, Const, k_0, p, rho, EE, GG, HH)
c = 3e10;
m  = [-N_max:N_max];
if(N_max == 0) m = 0; end
nu = m;
...
q = q.* (2*(imag(q) <= 0)-1);
    ...
    ElectricField = -(Const(1)*((n1*p+GG)*Jm1Q1/EE+alp1*N_max*Jm0Q1/(D1*rho))+Const(2)*((n2*p+GG)*Jm1Q2/EE+alp2*N_max*Jm0Q2/(D2*rho)));    
end
